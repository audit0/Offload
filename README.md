...
**Разобрать.** Кнопка «Начать» — и Offload сам раскладывает найденное по вопросам, на каждый нужно ответить «да» или «не сейчас». Выбирать файлы и ходить по папкам не нужно. Вопросы такие: «Удалить мусор?» — места из списка Offload, которые программы пересоздают сами (кеши сборки Xcode, пакеты Homebrew, npm, pip, Yarn, Cargo, Gradle, CocoaPods, Go, кеши Chrome, Spotify, VS Code и сред JetBrains, прошивки iPhone); «Очистить Docker?» — кеш сборки и образы без имени, остатки пересборок (образы с именем, тома и контейнеры не трогаются; все неиспользуемые образы можно убрать в разделе «Docker»); «Удалить лишние копии?», «Удалить старые установщики?», «Убрать в сейф крупное и старое?» (со сверкой каждого файла), «Добавить проекты в бэкап?». Виртуальные машины UTM разбор не удаляет: удалять и переносить их нужно в самом UTM, иначе он их потеряет, — как это сделать, подсказывает «Освободить место» → «Как освободить…». У каждого вопроса — сколько освободится и одна строка о том, что в нём; полный список — по «Что именно». «Разрешить всё» отвечает «да» на всё, кроме установщиков: их удаляют только отдельным «да» на их вопрос, посмотрев список. Сделанное по «да» видно сразу у вопроса. Удаляемое уходит в Корзину: «Вернуть» у вопроса возвращает его на прежние места, а в конце Offload спросит, удалить ли это из Корзины насовсем, чтобы место освободилось сразу; сколько освободилось, он меряет по свободному месту на диске, а не складывает размеры. Кеш открытой программы в вопрос не входит. «Не сейчас» ничего не запоминает — в следующий раз вопрос будет снова; то, что вы вернули из Корзины, Offload больше не предлагает. Правый щелчок по строке в «Что именно» → «Не предлагать больше» — и объект (папка — со всем, что внутри) в разбор больше не попадает; список таких — на первой странице, вернуть можно в любой момент.

Одинаковые файлы сверяются по размеру, началу и концу файла и SHA-256 всего содержимого. Остаётся одна копия: та, что лежит на своём месте, а не в Загрузках, с именем без «(1)» и появившаяся раньше; лишние перед удалением ещё раз сверяются с ней байт в байт. Медиатеки «Музыки» и «TV», пакеты, git-проекты и файлы iCloud, которых нет на Mac, поиск не трогает. Клоны APFS, которые места не занимают, не предлагает. Зашифрованные образы `.dmg` и `.iso` (и их копии) удалить из разбора нельзя; старые установщики удаляются только по отдельному «да» на их вопрос. Решения запоминаются в базе на Mac (SQLite): тот же объект в следующий раз отмечен так же. Когда подключаете внешний диск, «Обзор» предлагает разобрать Mac.
