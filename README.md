...
<table>
<tr>
<td><img src="docs/screenshots/cleanup.png" alt="Разобрать: что в Корзину, что в сейф, что в бэкап — с объяснением у каждой строки"></td>
...
**Разобрать.** Одна кнопка — и Offload раскладывает найденное: что удалить, что убрать в сейф, что добавить в бэкап, что оставить, — и объясняет каждое предложение. Удаление — только в Корзину и только то, что пересоздаётся само или скачивается заново (кеши сборки Xcode, пакеты Homebrew, pip, npm); старые установщики Offload только помечает, удалить их решаете вы, а зашифрованные образы `.dmg` и `.iso` удалить из разбора нельзя; личные файлы можно только убрать в сейф со сверкой. Исключение — лишние копии одинаковых файлов. Совпадение проверяется по размеру, началу и концу файла и SHA-256 всего содержимого. Остаётся одна копия: та, что лежит на своём месте, а не в Загрузках, с именем без «(1)» и появившаяся раньше. Остальные уходят в Корзину, каждая после побайтовой сверки с остающейся. Медиатеки «Музыки» и «TV», пакеты, git-проекты и файлы iCloud, которых нет на Mac, поиск не трогает. Клоны APFS, которые места не занимают, не предлагает. Копии зашифрованных `.dmg` и образов `.iso`, как и сами образы, не удаляются; одинаковые старые установщики показываются каждый своей строкой — удалить их решаете вы. В каждой строке выбор можно поменять, и ничего не происходит без подтверждения. Решения запоминаются в базе на Mac (SQLite), и в следующий раз предложение начинается с них. Когда подключаете внешний диск, «Обзор» предлагает разобрать Mac.

**Привычки.** Offload учится на ваших решениях — только на этом Mac и без сети. Похожим считается то, что лежит в той же папке домашней, того же вида (папка, проект с git, видео, документ, копия одинакового файла…), примерно того же размера и давности. Когда похожее вы хотя бы трижды решили одинаково и так решены не меньше трёх четвертей похожих случаев, Offload предлагает то же, ставит пометку «как вы обычно» и объясняет: «Похожее вы обычно оставляете (4 из 4): папки в «Документах» от 1 до 10 ГБ». Если самые похожие решения расходятся, решают правила. Учитывается только ваш выбор: «оставить» там, где оставить и предлагалось, не считается — такую строку можно было и не увидеть. Удалить привычка не предлагает никогда: в Корзину предлагают только правила (то, что пересоздаётся само) и ваше решение по тому же файлу — установщики вы удаляете сами, сколько бы похожих ни удаляли раньше. Привычка может предложить оставить, убрать в сейф или добавить в бэкап, а лишнюю копию одинакового файла — оставить. Чему Offload научился, видно на первой странице «Разобрать»; там же кнопка «Забыть мои решения».
