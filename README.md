...
<table>
<tr>
<td><img src="docs/screenshots/cleanup.png" alt="Разобрать: вопросы о найденном — удалить мусор, очистить Docker, удалить лишние копии — с ответами «да» и «не сейчас»"></td>
...
Когда шаги пройдены, дальше хватает одной кнопки: подключили диск — «Разобрать» → «Начать» — ответили на вопросы «да» или «не сейчас».
...
**Разобрать.** Кнопка «Начать» — и Offload сам раскладывает найденное по вопросам, на каждый нужно ответить «да» или «не сейчас». Выбирать файлы и ходить по папкам не нужно. Вопросы такие: «Удалить мусор?» — места из списка Offload, которые программы пересоздают сами (кеши сборки Xcode, пакеты Homebrew, npm, pip, Yarn, Cargo, Gradle, CocoaPods, Go, кеши Chrome, Spotify, VS Code и сред JetBrains, прошивки iPhone); «Очистить Docker?» — кеш сборки и образы, которые не нужны ни одному контейнеру (тома и контейнеры не трогаются); «Удалить лишние копии?», «Удалить старые установщики?», «Убрать в сейф крупное и старое?» (со сверкой каждого файла), «Добавить проекты в бэкап?» — и отдельно о каждой виртуальной машине UTM, которую не запускали месяц и дольше. У каждого вопроса — сколько освободится и одна строка о том, что в нём; полный список — по «Что именно». «Разрешить всё» отвечает «да» на всё, кроме машин: о каждой машине — только отдельным ответом. Сделанное по «да» видно сразу у вопроса. Удаляемое уходит в Корзину: «Вернуть» у вопроса возвращает его на прежние места, а в конце Offload спросит, удалить ли это из Корзины насовсем, чтобы место освободилось сразу; сколько освободилось, он меряет по свободному месту на диске, а не складывает размеры. Кеш открытой программы в вопрос не входит, а машину, пока открыт UTM, Offload не удаляет. «Не сейчас» ничего не запоминает — в следующий раз вопрос будет снова; машину, которую вы решили оставить, и то, что вы вернули из Корзины, Offload больше не предлагает. Правый щелчок по строке в «Что именно» → «Не предлагать больше» — и объект (папка — со всем, что внутри) в разбор больше не попадает; список таких — на первой странице, вернуть можно в любой момент.
...
**Привычки.** Offload учится на ваших решениях — только на этом Mac и без сети. Похожим считается то, что лежит в той же папке домашней, того же вида (папка, проект с git, видео, документ, копия одинакового файла…), примерно того же размера и давности. Когда похожее вы хотя бы трижды решили одинаково и так решены не меньше трёх четвертей похожих случаев, Offload поступает с похожим так же — кладёт его в тот же вопрос или не спрашивает о нём вовсе, — ставит в «Что именно» пометку «как вы обычно» и объясняет: «Похожее вы обычно убираете в сейф (5 из 5): папки в «Фильмах» больше 10 ГБ». Если самые похожие решения расходятся, решают правила. Учитывается только ваш выбор: «да» и то, что вы вернули из Корзины; «не сейчас» не учит. В вопросы об удалении привычка ничего не добавляет: удалить предлагается только мусор по правилам, лишние копии, старые установщики и давно не запускавшиеся машины. Привычка может добавить похожее в вопрос о сейфе или бэкапе, а лишнюю копию одинакового файла — оставить. Чему Offload научился, видно на первой странице «Разобрать»; там же кнопка «Забыть мои решения».
...
**Docker.** Сколько внутри Docker занимают образы, кеш сборки, контейнеры и тома. «Освободить место…» удаляет кеш сборки и образы, которые не нужны ни одному контейнеру, а остановленные контейнеры — только если их отметить, и показывает, насколько после этого уменьшился `Docker.raw` на Mac. Тома очистка не трогает: в них данные. Список томов — с размером и контейнерами, которые их используют; неиспользуемые тома упаковываются в сейф (или на диск) со сверкой и возвращаются одной кнопкой.
