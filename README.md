...
**Обзор.** Свободное место на диске Mac, давление памяти, swap, сжатая память, время без перезагрузки и какие приложения сколько занимают. Советы на человеческом языке: например, что swap переполнен и Mac пора перезагрузить.
...
**Перенос на внешний диск.** Копирование → перечитывание копии и сверка SHA-256 → проверка, что оригинал не менялся → удаление оригинала. Рядом с архивом кладутся список контрольных сумм (совместим с `shasum -c`) и права доступа файлов. Всё записывается в журнал на самом диске, и перенесённое можно вернуть обратно на любом Mac. Переносы, сделанные раньше без Offload, можно добавить в журнал вручную — возвращаются они так же, со сверкой.

**Бэкап.** Обновляемая копия папок с проектами: копируются только новые и изменившиеся файлы, каждый сверяется. Папку бэкапа на диске можно выбрать, в том числе уже существующую. `node_modules`, `.venv`, `dist` и другие восстанавливаемые папки пропускаются. Файлы с секретами (`.env`, `*.pem`, `*.key`, `id_ed25519`…) в открытый бэкап не попадают никогда.

**Шифрованный контейнер.** Sparsebundle с AES-256 на внешнем диске для `~/.ssh`, учётки GitHub CLI, дотфайлов с токенами и секретов из проектов. Пароль задаёте вы, Offload его не хранит. Уже существующий зашифрованный контейнер на диске Offload подхватит и дополнит, не трогая ваших заметок и папок внутри. Заодно предупреждает о SSH-ключах без парольной фразы.
...
- **exFAT** не хранит права доступа и расширенные атрибуты. Права сохраняются рядом с архивом и возвращаются при восстановлении, а служебные файлы `._*`, которые создаёт macOS, удаляются после сверки. Разрежённые файлы учитываются по полному размеру.
...
Нужны только Xcode Command Line Tools (Swift 6). Сам Xcode не нужен.

Одна оговорка: начиная с SDK macOS 27 обёртки SwiftUI (`@State` и другие) стали макросами, а их плагин поставляется только с Xcode. Если у вас стоят одни Command Line Tools версии 27 и новее, `scripts/build-app.sh` сам соберёт приложение против SDK 26 из их состава, а если такого SDK нет — скажет, что нужен Xcode.
...
Build from source with Xcode Command Line Tools: `swift run OffloadChecks`, then `scripts/install-local.sh`. The UI is in Russian. See [SECURITY.md](SECURITY.md) for the threat model. MIT licensed.
